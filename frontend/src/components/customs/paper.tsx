...
/**
 * paper.tsx — "_projectX: a signed-mandate checkpoint for agentic payments,"
...
import { GhostButton, InkButton, Reveal, inr } from "./bits";
    ...
    (async () => {
      try {
        ...
        setStats({
          ...
          refusedCount: l.filter((e: any) => e.status === "VIOLATION").length,
          attackCount: m.blocked_violations,
          netPaise: m.total_savings_generated_inr * 100,
          aiCostPaise: 0,
          eventsTotal: l.length,
          chain: { ok: m.pqc_status?.includes("ACTIVE") ?? false, length: l.length },
        });
      } catch {
      ...
      <article className="sheet rounded-[6px] px-6 py-10 sm:px-14 sm:py-12">
        ...
        <header className="border-b border-line pb-8">
          ...
          <h1 className="mt-5 font-display text-[clamp(28px,4.2vw,42px)] font-semibold leading-[1.12] tracking-[-0.015em] text-ink">
            _projectX: a signed-mandate checkpoint for agentic payments
          </h1>
        ...
        <section aria-label="abstract" className="mt-10">
          ...
          <p className="mt-3 text-[15.5px] leading-[1.8] text-inksoft">
            ...
            present any of these. We present <strong className="font-semibold text-ink">_projectX</strong>,
        ...
        <PaperSection n="2" title="Approach: mandates, not credentials">
          <p>
            _projectX gives the agent neither a card token nor keys to a wallet. The
...
"z3Fq…9cA" (detached; verified at every bind)`}</code></pre>
          <p className="mt-2 font-mono text-[10px] text-inksoft">
            illustrative shape — the schema and reason codes are the contract in src/lib/_projectX/gate/types.ts
          </p>
          <p>
        ...
        <PaperSection n="4" title="The decision pipeline">
          <p>
            ...
            signature, signature precedes policy, policy precedes price — cheap checks
        ...
        <PaperSection n="5" title="Economics: the channel P&L">
          <p>
            A growth channel is adopted on unit economics, not novelty. _projectX
        ...
        <section aria-label="references" className="mt-10 border-t border-line pt-8">
          ...
          <ul className="mt-3 space-y-1.5 text-[12.5px] leading-relaxed text-inksoft">
            ...
            <li>[2] _projectX, <span className="font-mono text-[11px]">JUDGE.md</span> — every claim mapped to a file and a regeneration command. (this repo)</li>
            <li>[3] _projectX, <span className="font-mono text-[11px]">ARCHITECTURE.md</span> — the decision table and the P&amp;L formula. (this repo)</li>
            <li>[4] _projectX, <span className="font-mono text-[11px]">ENGINEERING_LOG.md</span> — dated incidents, including D5-1. (this repo)</li>
            <li>[5] _projectX, <span className="font-mono text-[11px]">results/</span> — measured artifacts: conformance_matrix, ablation, cost_meter, project, audit_chain. (this repo)</li>
          </ul>
        </section>
      </article>
      ...
      <Reveal>
        ...
        <p className="mx-auto mt-24 pb-8 max-w-2xl text-center font-mono text-[10.5px] leading-relaxed text-inksoft">
          Read-only · anon key is SELECT-only behind row-level security · the service-role key never reaches this app
        </p>
      </Reveal>
    </div>
  );
