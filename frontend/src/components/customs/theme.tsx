...
export function ThemeToggle({ className }: { className?: string }) {
  ...
  const toggle = () => {
    ...
    try {
      localStorage.setItem("_projectX-theme", next ? "light" : "dark");
    } catch {
...
export function SystemThemeAsk() {
  ...
  useEffect(() => {
    try {
      if (localStorage.getItem("_projectX-theme")) return;
      if (localStorage.getItem("_projectX-theme-ask") === "done") return;
  ...
  const settle = (dark: boolean) => {
    ...
    try {
      localStorage.setItem("_projectX-theme", dark ? "dark" : "light");
      localStorage.setItem("_projectX-theme-ask", "done");
    } catch {
