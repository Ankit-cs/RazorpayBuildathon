...
import type { ChatEvent as ChatEventT } from "@/lib/_projectX/agent/loop";
import type { Product } from "@/lib/_projectX/store/catalog";
...
function ReceiptCard({ event }: { event: Extract<ChatEventT, { kind: "receipt" }> }) {
  ...
  const receiptText = [
    ...
    `cleared by _projectX · hash-chained in ledger`,
  ].join("\n");
