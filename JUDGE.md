...
Razorpay's homepage sells agents shopping in your app. ProjectX ships both sides
...
The product itself is live: **https://ProjectX.srivtx.xyz** (or `make demo`
...
one app: Playground (`src/components/ProjectX/playground.tsx`) and Control Room
(`src/components/ProjectX/control-room.tsx`) · the **Why it exists** view
(`src/components/ProjectX/why.tsx`) states the problem and the unbuilt merchant
...
key is present.
Evidence: `src/lib/ProjectX/gate/decide.ts` (the checklist, plain code) ·
...
JSONL — the audit trail IS the database) · `src/components/ProjectX/why.tsx` ·
`src/components/ProjectX/paper.tsx` + `PAPER.md` (twins, AGENTS invariant 11).
...
price re-verification at bind (`src/lib/ProjectX/gate/`); **gated** — authored
attack corpus refused with reason codes, human desk over ₹10,000
(`src/lib/ProjectX/fuzz/corpus.ts`); **audit trail** — hash-chained JSONL with a
tamper control (`src/lib/ProjectX/ledger/`); **one failure handled gracefully** —
...
- Live deployment: **https://ProjectX.srivtx.xyz** — `/api/health` answers
...
- Closest prior art we found: EACP (https://github.com/HRaj07/eacp) — a gated
  ...
  is "generating a Razorpay Payment Link for a human to act on." ProjectX
