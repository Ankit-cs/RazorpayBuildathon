...
The repo is **already live at https://github.com/Ankit-cs/RazorpayBuildathon** (pushed from
...
| Thing | Where | State |
|---|---|---|
| Source of truth | the build machine's `repo-out/ProjectX/` + every tar in `download/` | 7 commits (design round on top of v1.1), all green locally |
| Public repo | `https://github.com/Ankit-cs/RazorpayBuildathon` | `main` pushed, CI: `verify` workflow (3 jobs) |
...
If you're starting from a tar: `tar xzf ProjectX-repo-<date>.tar.gz && cd ProjectX`.
...
3. If you need to push again, mint a new fine-grained PAT scoped to **only
   `ProjectX`**, permission `Contents: Read and write`, expiry short.
...
git push https://<user>:<PAT>@github.com/Ankit-cs/RazorpayBuildathon.git main
# or: git push origin main               # origin is already set, no token stored
git remote -v                            # must show the clean URL, no token
```

`git remote -v` must print `https://github.com/Ankit-cs/RazorpayBuildathon.git` — no token
in the URL, no token in `.git/config`, no token in any file. If a token ever
lands in a URL, `git remote set-url origin https://github.com/Ankit-cs/RazorpayBuildathon.git`.
...
```
https://vercel.com/new/clone?repository-url=https://github.com/Ankit-cs/RazorpayBuildathon
```
