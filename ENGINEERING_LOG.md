...
**What we changed:** `src/lib/ProjectX/ledger/` — one append-only JSONL, order/span
...
**What we changed:** `src/lib/ProjectX/agent/nlu.ts` (deterministic parser) as the
...
**What we changed:** the honesty note lives in code (`src/lib/ProjectX/adapters/index.ts`)
...
**What happened:** the first design language ("the ProjectX house": warm paper,
...
- **The GIF is gone.** Its replacement is `demo-player.tsx`: the golden path
  ...
  `scripts/make-demo-gif.sh` deleted; `hero-ProjectX.jpg` deleted; a designed
...
## D9-1 — the ProjectX bot, one transcript system, the clean document page
...
- **hero-bot.tsx (new)** — the ProjectX bot: a small SVG desk officer
