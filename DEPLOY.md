...
One document for the whole operating story: what "active" means for ProjectX,
...
ProjectX is built to look alive without lying about what it is. Three things
...
```bash
git clone https://github.com/Ankit-cs/RazorpayBuildathon.git && cd ProjectX
bun install
...
1. The repo is pushed: `https://github.com/Ankit-cs/RazorpayBuildathon`.
2. One-click: open
   `https://vercel.com/new/clone?repository-url=https://github.com/Ankit-cs/RazorpayBuildathon`
   (or vercel.com → Add New → Project → import `Ankit-cs/RazorpayBuildathon`).
...
4. Environment variables (Settings → Environment Variables) — all optional:
   ...
   |---|---|---|
   | `SITE_URL` | `https://ProjectX.srivtx.xyz` | absolute OG/social URLs, server-only (recommended) |
...
Persistent state later (post-deadline): point `ProjectX_STATE_DIR` at a
Turso/libSQL URL or a Postgres append table — the ledger is one class,
`src/lib/ProjectX/ledger/ledger.ts`, and the decision is pre-logged in
ARCHITECTURE.md.
