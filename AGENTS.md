...
- **A protocol adapter**: implement the transport in
  `src/lib/ProjectX/adapters/index.ts` following the MCP/ACP pattern (same tool
...
- **A fuzz case**: one entry in `src/lib/ProjectX/fuzz/corpus.ts` — deterministic
...
- **A measured number**: write the harness in `scripts/`, make it emit
  ...
  produces it.
- **A catalog product**: one entry in `src/lib/ProjectX/store/catalog.ts`
